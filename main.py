...
@app.get("/")
def welcome():
    return "Bienvenidos a mi primer PI, aquí podra encontrar diferentes peliculas y series de 4 plataformas "

@app.get("/menu")
def menu():
    return "Las funciones que encontrará aquí son las siguientes: get_max_duration, get_score_count, get_count_platform, get_actor"


#---QUERIES---
#Primer consigna: pelicula con mayor duración
...
def get_max_duration(year: Optional[int] = None, platform: Optional[str] = None, duration_type: Optional[str] = 'min'):
    ...
    if duration_type is not None and duration_type not in['min', 'season']:
        return('Los valores válidos son min y season')

    if year is None:
        return('Ingrese un año')
    elif not isinstance(year, int):
        return('El año ingresado debe ser entero')
    
    canal= df[df['movieId'].str.contains(platform[0], case= False)]
...
def get_count_platform(platform: str):
    ...
    filtro= df[df['movieId'].str.contains(platform[0], case= False)]
